...
- 英雄详情补充 78 位英雄的移动分类、基础移速、复活与回血时序参数、主攻击冷却/距离，以及已核实的移动与被动条目。基础参数不等于满级实际表现，技能、形态和专属脚本可覆盖这些数值。
- 常驻飞行只采用英雄自身的飞行标记；技能传送、赶路变形、跳跃、视觉悬浮分别说明。调动距离使用脚本的严格边界，技能解锁与地形触发条件一并保留。
- 英雄机制说明在 `tools/hero-details.mjs` 维护，并用 `tools/hero-details-review.json` 的源码指纹保护。同步不会自动更新审阅指纹；来源变化会将相关条目标记为待核实并移出移动分类筛选。未列出的专属机制不代表不存在。
- `npm run sync:dove -- --heroes-only` 仅更新英雄档案和英雄图像，保留其他资料与更新历史；英雄来源版本、提交和同步时间独立记录于 `metadata.heroSnapshot`，在英雄页显示。当前英雄来源为 2.0.7.7，原有其他资料保持 2.0.7.6。
